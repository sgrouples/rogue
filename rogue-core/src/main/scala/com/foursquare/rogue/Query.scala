...
case class Query[M, R, +State](
  ...
  def not[F](clause: M => QueryClause[F]) =
    addClause(clause, expectedIndexBehavior = DocumentScan, negated = true)
  ...
  def select(n: Int) = {
    ...
    val code = """
    def select[%s, S2](%s)
          (implicit ev: AddSelect[State, %s]): Query[M, %s, S2] = {
      selectCase(%s, (%s) => %s)
    }"""
  ...
  def selectCase(n: Int) = {
    ...
    def selectCase[%s, CC, S2](%s,
          create: %s => CC)(implicit ev: AddSelect[State, %s]): Query[M, CC, S2] = {
      val inst = meta
      val fields = List(%s)
